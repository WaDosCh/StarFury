...
public class SpaceGame {
	...
	private FloatBuffer shotsVertices = BufferUtils
			.createFloatBuffer(6 * 6 * maxShots);
	private FloatBuffer particleVertices = BufferUtils
			.createFloatBuffer(6 * 6 * maxParticles);
	private FloatBuffer crosshairVertices = BufferUtils
			.createFloatBuffer(6 * 2);
	...
	private void init() throws IOException {
		...
		window = glfwCreateWindow(width, height, "Little Space Shooter Game",
		...
		glfwSetFramebufferSizeCallback(window,
				fbCallback = new GLFWFramebufferSizeCallback() {
					public void invoke(long window, int width, int height) {
						if (width > 0
								&& height > 0
								&& (SpaceGame.this.width != width || SpaceGame.this.height != height)) {
		...
		System.out
				.println("Hold the right mouse button to rotate towards the mouse cursor");
		...
		glfwSetCursorPosCallback(window,
				cpCallback = new GLFWCursorPosCallback() {
					public void invoke(long window, double xpos, double ypos) {
						...
						SpaceGame.this.mouseX = Math.max(-width / 2.0f,
								Math.min(width / 2.0f, normX));
						SpaceGame.this.mouseY = Math.max(-height / 2.0f,
								Math.min(height / 2.0f, normY));
					}
				});
	...
	private void createCubemapProgram() throws IOException {
		int vshader = createShader("org/lwjgl/demo/game/cubemap.vs",
				GL_VERTEX_SHADER);
		int fshader = createShader("org/lwjgl/demo/game/cubemap.fs",
				GL_FRAGMENT_SHADER);
		int program = createProgram(vshader, fshader);
		glUseProgram(program);
		int texLocation = glGetUniformLocation(program, "tex");
		glUniform1i(texLocation, 0);
		cubemap_invViewProjUniform = glGetUniformLocation(program,
				"invViewProj");
		glUseProgram(0);
	...
	private void createShipProgram() throws IOException {
		int vshader = createShader("org/lwjgl/demo/game/ship.vs",
				GL_VERTEX_SHADER);
		int fshader = createShader("org/lwjgl/demo/game/ship.fs",
				GL_FRAGMENT_SHADER);
		int program = createProgram(vshader, fshader);
		glUseProgram(program);
	...
	private void createParticleProgram() throws IOException {
		int vshader = createShader("org/lwjgl/demo/game/particle.vs",
				GL_VERTEX_SHADER);
		int fshader = createShader("org/lwjgl/demo/game/particle.fs",
				GL_FRAGMENT_SHADER);
		int program = createProgram(vshader, fshader);
		glUseProgram(program);
	...
	private void createShotProgram() throws IOException {
		int vshader = createShader("org/lwjgl/demo/game/shot.vs",
				GL_VERTEX_SHADER);
		int fshader = createShader("org/lwjgl/demo/game/shot.fs",
				GL_FRAGMENT_SHADER);
		int program = createProgram(vshader, fshader);
		glUseProgram(program);
	...
	private void createCubemapTexture() throws IOException {
		...
		for (int i = 0; i < 6; i++) {
			imageBuffer = ioResourceToByteBuffer("org/lwjgl/demo/space_"
					+ names[i] + (i + 1) + ".jpg", 8 * 1024);
			...
			glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB8,
					w.get(0), h.get(0), 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	...
	private void update() {
		...
		projMatrix.setPerspective((float) Math.toRadians(40.0f), (float) width
				/ height, 0.1f, 5000.0f);
		viewMatrix.set(cam.rotation).invert(invViewMatrix);
		viewProjMatrix.set(projMatrix).mul(viewMatrix)
				.invert(invViewProjMatrix);
		frustumIntersection.set(viewProjMatrix);

		/* Update the background shader */
		glUseProgram(cubemapProgram);
		glUniformMatrix4fv(cubemap_invViewProjUniform, false,
				invViewProjMatrix.get(matrixBuffer));

		/* Update the ship shader */
		glUseProgram(shipProgram);
		glUniformMatrix4fv(ship_viewUniform, false,
				viewMatrix.get(matrixBuffer));
		glUniformMatrix4fv(ship_projUniform, false,
		...
		if (leftMouseDown
				&& (thisTime - lastShotTime >= 1E6 * shotMilliseconds)) {
			shoot();
	...
	private void updateControls() {
		...
		if (keyDown[GLFW_KEY_LEFT_CONTROL])
			cam.linearAcc.fma(-straveThrusterAccFactor, cam.up(tmp2));
		if (rightMouseDown)
			cam.angularAcc.set(2.0f * mouseY * mouseY * mouseY, 2.0f * mouseX
					* mouseX * mouseX, rotZ);
	...
	private static Vector3f intercept(Vector3d shotOrigin, float shotSpeed,
			Vector3d targetOrigin, Vector3f targetVel, Vector3f out) {
		...
		float len = (float) Math.sqrt(dirToTargetX * dirToTargetX
				+ dirToTargetY * dirToTargetY + dirToTargetZ * dirToTargetZ);
		dirToTargetX /= len;
		dirToTargetY /= len;
		dirToTargetZ /= len;
		float targetVelOrthDot = targetVel.x * dirToTargetX + targetVel.y
				* dirToTargetY + targetVel.z * dirToTargetZ;
		...
		float shotVelSpeed = (float) Math.sqrt(targetVelTangX * targetVelTangX
				+ targetVelTangY * targetVelTangY + targetVelTangZ
				* targetVelTangZ);
		if (shotVelSpeed > shotSpeed) {
			return null;
		}
		float shotSpeedOrth = (float) Math.sqrt(shotSpeed * shotSpeed
				- shotVelSpeed * shotVelSpeed);
		...
		return out.set(shotVelOrthX + targetVelTangX,
				shotVelOrthY + targetVelTangY, shotVelOrthZ + targetVelTangZ)
				.normalize();
	}
	...
	private void shootFromShip(long thisTime, int index) {
		...
		Vector3d shotPos = tmp.set(ship.x, ship.y, ship.z).sub(cam.position)
				.negate().normalize().mul(1.01f * shipRadius)
				.add(ship.x, ship.y, ship.z);
		Vector3f icept = intercept(shotPos, shotVelocity, cam.position,
				cam.linearVel, tmp2);
		if (icept == null)
			return;
	...
	private void shoot() {
		...
		for (int i = 0; i < projectilePositions.length; i++) {
			Vector3d projectilePosition = projectilePositions[i];
			Vector4f projectileVelocity = projectileVelocities[i];
			invViewProjMatrix.transformProject(tmp2.set(mouseX, -mouseY, 1.0f))
					.normalize();
			if (projectileVelocity.w <= 0.0f) {
				...
				projectileVelocity.w = 0.01f;
				if (!firstShot) {
					projectilePosition.set(cam.right(tmp3)).mul(shotSeparation)
							.add(cam.position);
					firstShot = true;
				} else {
					projectilePosition.set(cam.right(tmp3))
							.mul(-shotSeparation).add(cam.position);
					break;
				}
			}
		}
	...
	private void drawShips() {
		...
		for (int i = 0; i < ships.length; i++) {
			...
			if (frustumIntersection.testSphere(x, y, z, shipRadius)) {
				...
				glUniformMatrix4fv(ship_modelUniform, false,
		...
		for (int i = 0; i < particlePositions.length; i++) {
			...
			if (particleVelocity.w > 0.0f) {
				...
				if (frustumIntersection.testPoint(x, y, z)) {
					...
					particleVertices.put(tmp2.x - particleSize)
							.put(tmp2.y - particleSize).put(tmp2.z).put(w)
							.put(-1).put(-1);
					particleVertices.put(tmp2.x + particleSize)
							.put(tmp2.y - particleSize).put(tmp2.z).put(w)
							.put(1).put(-1);
		...
		for (int i = 0; i < projectilePositions.length; i++) {
			...
			if (projectileVelocity.w > 0.0f) {
				...
				if (frustumIntersection.testPoint(x, y, z)) {
					...
					viewMatrix.transformPosition(tmp2.set(x, y, z));
					shotsVertices.put(tmp2.x - shotSize).put(tmp2.y - shotSize)
							.put(tmp2.z).put(w).put(-1).put(-1);
					shotsVertices.put(tmp2.x + shotSize).put(tmp2.y - shotSize)
							.put(tmp2.z).put(w).put(1).put(-1);
					shotsVertices.put(tmp2.x + shotSize).put(tmp2.y + shotSize)
							.put(tmp2.z).put(w).put(1).put(1);
	...
	private void drawVelocityCompass() {
		...
		glColor4f(1, 1, 1, 1);
		glVertex3f(0, 0, 0);
		glVertex3f(cam.linearVel.x / maxLinearVel, cam.linearVel.y
				/ maxLinearVel, cam.linearVel.z / maxLinearVel);
		glEnd();
		glPopMatrix();
		glMatrixMode(GL_PROJECTION);
		glPopMatrix();
	...
	private void drawHudShotDirection() {
		...
		Vector3f interceptorDir = intercept(cam.position, shotVelocity,
				targetOrigin, tmp3.set(cam.linearVel).negate(), tmp2);
	...
	private void drawHudShip() {
		...
		int quads = stb_easy_font_print(0, 0,
				Integer.toString((int) (tmp3.length())), null, charBuffer);
	...
	private boolean narrowphase(FloatBuffer data, double x, double y, double z,
			float scale, Vector3d pOld, Vector3d pNew,
			Vector3d intersectionPoint, Vector3f normal) {
		...
		while (data.hasRemaining() && !intersects) {
			...
			float v2Y = data.get();
			float v2Z = data.get();
			if (Intersectionf.intersectLineSegmentTriangle(tmp2.x, tmp2.y,
					tmp2.z, tmp3.x, tmp3.y, tmp3.z, v0X, v0Y, v0Z, v1X, v1Y,
					v1Z, v2X, v2Y, v2Z, 1E-6f, tmp2)) {
				intersectionPoint.x = tmp2.x * scale + x;
				intersectionPoint.y = tmp2.y * scale + y;
				intersectionPoint.z = tmp2.z * scale + z;
				GeometryUtils.normal(v0X, v0Y, v0Z, v1X, v1Y, v1Z, v2X, v2Y,
						v2Z, normal);
				intersects = true;
			}
		}
	...
	private static boolean broadphase(double x, double y, double z,
			float boundingRadius, float scale, Vector3d pOld, Vector3d pNew) {
		return Intersectiond.testLineSegmentSphere(pOld.x, pOld.y, pOld.z,
				pNew.x, pNew.y, pNew.z, x, y, z, boundingRadius
						* boundingRadius * scale * scale);
	}
	...
	private void updateParticles(float dt) {
		for (int i = 0; i < particlePositions.length; i++) {
			...
			Vector3d particlePosition = particlePositions[i];
			newPosition
					.set(particleVelocity.x, particleVelocity.y,
							particleVelocity.z).mul(dt).add(particlePosition);
	...
	private void updateShots(float dt) {
		projectiles: for (int i = 0; i < projectilePositions.length; i++) {
			...
			Vector3d projectilePosition = projectilePositions[i];
			newPosition
					.set(projectileVelocity.x, projectileVelocity.y,
							projectileVelocity.z).mul(dt)
			...
			for (int r = 0; r < shipCount; r++) {
				...
				if (ship == null)
					continue;
				if (broadphase(ship.x, ship.y, ship.z,
						this.ship.boundingSphereRadius, shipRadius,
						projectilePosition, newPosition)
						&& narrowphase(this.ship.positions, ship.x, ship.y,
								ship.z, shipRadius, projectilePosition,
								newPosition, tmp, tmp2)) {
			...
			for (int r = 0; r < asteroidCount; r++) {
				...
				if (asteroid == null)
					continue;
				if (broadphase(asteroid.x, asteroid.y, asteroid.z,
						this.asteroid.boundingSphereRadius, asteroid.scale,
						projectilePosition, newPosition)
						&& narrowphase(this.asteroid.positions, asteroid.x,
								asteroid.y, asteroid.z, asteroid.scale,
								projectilePosition, newPosition, tmp, tmp2)) {
