...
public class WavefrontMeshLoader {
	...
	public class MeshObject {
		...
		public int first;
		public int count;
		public Vector3f min = new Vector3f(Float.MAX_VALUE, Float.MAX_VALUE,
				Float.MAX_VALUE);
		public Vector3f max = new Vector3f(Float.MIN_VALUE, Float.MIN_VALUE,
				Float.MIN_VALUE);
	...
	private static WavefrontInfo getInfo(BufferedReader reader)
			throws IOException {
		String line = "";
	...
	private static byte[] readSingleFileZip(String zipResource)
			throws IOException {
		ZipInputStream zipStream = new ZipInputStream(WavefrontMeshLoader.class
				.getClassLoader()
				.getResourceAsStream("resource/" + zipResource));
	...
	public Mesh loadMesh(String resource) throws IOException {
		...
		WavefrontInfo info = getInfo(new BufferedReader(new InputStreamReader(
				new ByteArrayInputStream(arr))));

		// Allocate buffers for all vertices/normal
		ByteBuffer positionByteBuffer = BufferUtils
				.createByteBuffer(3 * info.numberOfVertices * 4);
		ByteBuffer normalByteBuffer = BufferUtils
				.createByteBuffer(3 * info.numberOfNormals * 4);
		...
		ByteBuffer positionDataByteBuffer = BufferUtils
				.createByteBuffer((fourComponentPosition ? 4 : 3) * 3
						* info.numberOfFaces * 4);
		ByteBuffer normalDataByteBuffer = BufferUtils
				.createByteBuffer(3 * 3 * info.numberOfFaces * 4);
		...
		positionData.flip();
		normalData.flip();
		mesh.boundingSphereRadius = Math.max(maxX - minX,
				Math.max(maxY - minY, maxZ - minZ)) * 0.5f;
		mesh.positions = positionData;
		mesh.normals = normalData;
		mesh.numVertices = positionData.limit()
				/ (fourComponentPosition ? 4 : 3);
		return mesh;
	}
}
